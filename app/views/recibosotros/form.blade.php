...
<?php
    if ($recibos->exists):
        $form_data = array('route' => array('recibosotros.update', $recibos->id), 'method' => 'PATCH');
        $action    = 'Editar';
    else:
        $form_data = array('route' => 'recibosotros.store', 'method' => 'POST');
        $action    = 'Crear';        
    endif;
?>
@section ('title') {{ $action }} recibo otro | @stop
@section('content')
  
  <legend><h3 class="form-signin-heading">{{ $action }} recibo otro</h3></legend>
  ...
  <p>
    @if ($action == 'Editar')  
      {{ Form::model($recibos, array('route' => array('recibosotros.destroy', $recibos->id), 'method' => 'DELETE', 'role' => 'form')) }}
        <div class="row">
